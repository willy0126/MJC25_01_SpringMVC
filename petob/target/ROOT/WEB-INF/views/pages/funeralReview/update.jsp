    ...
    <div class="container">
        ...
        <div class="row">
            <div class="col-12">
                ...
                <%-- action 경로 수정 --%>
                <form id="updateForm" action="${pageContext.request.contextPath}/funeral-reviews/${post.id}/update" method="POST" enctype="multipart/form-data">
                    <div class="card mb-3">
                        ...
                        <div class="card-body">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="reviewTitle" name="reviewTitle" placeholder="제목을 입력하세요" value="${post.reviewTitle}">
                            </div>
                            <%--// 제목 --%>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" id="reviewContent" name="reviewContent" rows="5" placeholder="내용을 입력하세요">${post.reviewContent}</textarea>
                            </div>
                            <%--// 내용 --%>
                            ...
                            <div class="mb-3">
                                ...
                                <%-- 날짜 형식을 yyyy-MM-dd로 포맷팅해야 input type="date"에 올바르게 표시됨 --%>
                                <fmt:formatDate value="${post.funeralDate}" pattern="yyyy-MM-dd" var="formattedFuneralDate"/>
                                <input type="date" class="form-control" id="funeralDate" name="funeralDate" value="${formattedFuneralDate}">
                            </div>
                            <%--// 장례 날짜 --%>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="location" name="location" placeholder="장례식장 위치를 입력하세요" value="${post.location}">
                            </div>
                            <%--// 장소 --%>
                            ...
                            <c:if test="${not empty post.fileName}">
                                <div class="mb-3">
                                    <div class="mb-2">
                                        <%-- 다운로드 링크 경로 수정 --%>
                                        <span>현재 파일: <a href="${pageContext.request.contextPath}/funeral-reviews/${post.id}/download" class="btn btn-sm btn-outline-primary">${post.originalFileName}</a></span>
                                    </div>
                                    <div class="form-check">
                                        <%-- deleteFile 체크박스 value를 true/false가 아닌, 체크 시 true 값 전송되도록 --%>
                            ...
                            <div class="mb-3">
                                <label for="uploadFile" class="form-label">${not empty post.fileName ? '새 파일로 변경' : '첨부 파일'}</label>
                                <input type="file" class="form-control" id="uploadFile" name="uploadFile" accept="*/*">
                                <small class="form-text text-muted">
                                    <c:if test="${not empty post.fileName}">새 파일을 선택하면 기존 파일은 교체됩니다. </c:if>
                        ...
                        <div class="card-footer">
                            <div>
                                ...
                                <%-- 취소 버튼 경로 수정 --%>
                                <a href="${pageContext.request.contextPath}/funeral-reviews/${post.id}" class="btn btn-secondary">취소</a>
                            </div>
                        </div>
                    </div>
                </form>
                ...
                <c:if test="${empty post}">
                    ...
                    <div class="text-center">
                        <a href="${pageContext.request.contextPath}/funeral-reviews" class="btn btn-primary">목록으로</a>
                    </div>
                </c:if>
                <%--// 게시글 수정 --%>
            </div>
