...
public class AuthInterceptor implements HandlerInterceptor{
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (requestUri.startsWith(request.getContextPath() + "/funeral-reviews")) {
            ...
            if (requestUri.equals(request.getContextPath() + "/funeral-reviews") && request.getMethod().equalsIgnoreCase("GET")) {
                return true;
            }
            // 글 상세보기(/funeral-reviews/{id})는 누구나 가능하도록 허용 (필요에 따라 변경)
            // 정규 표현식을 사용하여 /funeral-reviews/숫자 형태의 경로를 체크
            if (requestUri.matches(request.getContextPath() + "/funeral-reviews/\\d+$") && request.getMethod().equalsIgnoreCase("GET")) {
                return true;
            }
            // 파일 다운로드(/funeral-reviews/{id}/download)는 누구나 가능하도록 허용 (필요에 따라 변경)
            if (requestUri.matches(request.getContextPath() + "/funeral-reviews/\\d+/download$") && request.getMethod().equalsIgnoreCase("GET")) {
                return true;
            }
        ...
        if (requestUri.equals(request.getContextPath() + "/logout")) { // /auth/logout 에서 /logout 으로 변경 (AuthController 확인 필요)
            if (userId == null) {
                response.sendRedirect(request.getContextPath() + "/login");
                return false;
            }
