                        ...
                        <div class="inquiry-content">
                            ...
                            <div class="search-section">
                                ...
                                <form id="searchForm" class="search-form" method="GET"
                                    action="<c:url value='/inquiry/list'/>">
                                    <!-- 키워드 검색 -->
                                    <div class="search-group">
                                        <label for="keyword" class="search-label">키워드</label>
                                        <input type="text" id="keyword" name="search" class="search-input"
                                    ...
                                    <div class="search-group">
                                        ...
                                        <select id="category" name="category" class="search-select">
                                            ...
                                            <option value="SERVICE" ${param.category eq 'SERVICE' ? 'selected' : '' }>
                                                서비스 문의</option>
                                            <option value="RESERVATION" ${param.category eq 'RESERVATION' ? 'selected'
                                                : '' }>예약 문의</option>
                                            <option value="PRICE" ${param.category eq 'PRICE' ? 'selected' : '' }>가격 문의
                                            </option>
                                            <option value="LOCATION" ${param.category eq 'LOCATION' ? 'selected' : '' }>
                                                지점 문의</option>
                                            <option value="COMPLAINT" ${param.category eq 'COMPLAINT' ? 'selected' : ''
                                                }>불만 접수</option>
                                            <option value="SUGGESTION" ${param.category eq 'SUGGESTION' ? 'selected'
                                                : '' }>개선 제안</option>
                                            <option value="ETC" ${param.category eq 'ETC' ? 'selected' : '' }>기타
                                            </option>
                                        </select>
                                    </div>
                            ...
                            <div class="table-container">
                                <div class="table-header">
                                    <h5>총 <span style="color: #2b6cb0;">${totalCount != null ? totalCount : 0}</span>건의
                                        문의</h5>
                                </div>
                                                        ...
                                                        <tr style="cursor: pointer;"
                                                            ...
                                                            <td class="text-center number-cell">
                                                                <c:set var="currentPageNumber"
                                                                    value="${totalCount - ((currentPage - 1) * pageSize) - status.index}" />
                                                                <span class="post-number">${currentPageNumber}</span>
                                                            </td>

                                                            <!-- 📝 제목 -->
                                                            <td>
                                                                <a href="javascript:void(0)" class="inquiry-title"
                                                                    data-inquiry-id="${inquiry.inquiryId}">
                                                                    <!-- 비밀글 아이콘 -->
                                                            ...
                                                            <td class="text-center">
                                                                <c:choose>
                                                                    <c:when test="${not empty inquiry.username}">
                                                                        <span
                                                                            class="username">${inquiry.username}</span>
                                                                    </c:when>
                                                                    <c:otherwise>
                                                            ...
                                                            <td class="text-center">
                                                                <c:choose>
                                                                    <c:when test="${inquiry.status == 'WAITING'}">
                                                                        <span
                                                                            class="status-badge status-waiting">답변대기</span>
                                                                    </c:when>
                                                                    <c:when test="${inquiry.status == 'PROCESSING'}">
                                                                        <span
                                                                            class="status-badge status-processing">처리중</span>
                                                                    </c:when>
                                                                    <c:when test="${inquiry.status == 'COMPLETED'}">
                                                                        <span
                                                                            class="status-badge status-completed">답변완료</span>
                                                                    </c:when>
                                                                    <c:when test="${inquiry.status == 'CLOSED'}">
                                                                        <span
                                                                            class="status-badge status-closed">종료</span>
                                                                    </c:when>
                                                                    <c:otherwise>
                                                                        <span
                                                                            class="status-badge status-waiting">답변대기</span>
                                                ...
                                                <nav aria-label="문의 목록 페이지 네비게이션">
                                                    <ul class="pagination">
                                                        ...
                                                        <c:if test="${currentPage > 1}">
                                                            <li class="page-item">
                                                                <a class="page-link"
                                                                    href="<c:url value='/inquiry/list?page=1'/>"
                                                                    aria-label="첫 페이지">
                                                        ...
                                                        <c:if test="${pagination.hasPrevGroup}">
                                                            <li class="page-item">
                                                                <a class="page-link"
                                                                    href="<c:url value='/inquiry/list?page=${pagination.startPage - 1}'/>"
                                                                    aria-label="이전">
                                                        ...
                                                        <c:forEach var="pageNum" begin="${pagination.startPage}"
                                                            ...
                                                            <li
                                                                ...
                                                                <a class="page-link"
                                                                    href="<c:url value='/inquiry/list?page=${pageNum}'/>">${pageNum}</a>
                                                            </li>
                                                        </c:forEach>
                                                        ...
                                                        <c:if test="${pagination.hasNextGroup}">
                                                            <li class="page-item">
                                                                <a class="page-link"
                                                                    href="<c:url value='/inquiry/list?page=${pagination.endPage + 1}'/>"
                                                                    aria-label="다음">
                                                        ...
                                                        <c:if test="${currentPage < totalPages}">
                                                            <li class="page-item">
                                                                <a class="page-link"
                                                                    href="<c:url value='/inquiry/list?page=${totalPages}'/>"
                                                                    aria-label="마지막 페이지">
