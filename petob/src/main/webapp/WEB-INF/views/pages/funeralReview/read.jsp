                ...
                <% pageContext.setAttribute("newLine", "\n" ); %>
                    ...
                    <body>
                                            ...
                                            <c:if test="${not empty post}">
                                                <div class="card mb-3">
                                                    ...
                                                    <div class="card-body">
                                                        <%-- 아이디, 이름 등 기본 정보 --%>
                                                            ...
                                                            <%-- 장례일, 장소 정보 --%>
                                                                <div class="mb-3 text-muted small">
                                                                    장례일:
                                                                    <fmt:formatDate value="${post.funeralDate}"
                                                                        pattern="yyyy-MM-dd" /> | 장소: ${post.location}
                                                                </div>
                                                                <%-- 작성일, 수정일 정보 --%>
                                                                    <div class="mb-3 text-muted small">
                                                                        작성일:
                                                                        ...
                                                                        <c:if test="${not empty post.fileName}">
                                                                            <div class="mb-3">
                                                                                첨부파일: <a
                                                                                    ...
                                                                                    class="btn btn-sm btn-outline-primary">${post.originalFileName}</a>
                                                                            </div>
                                                                        </c:if>

                                                                        <%--===여기에 첨부 이미지 표시 코드 추가===--%>
                                                                            <c:if test="${not empty post.fileName}">
                                                                                <c:set var="fileNameLower"
                                                                                    value="${fn:toLowerCase(post.fileName)}" />
                                                                                <c:if
                                                                                    test="${fn:endsWith(fileNameLower, '.jpg') || fn:endsWith(fileNameLower, '.jpeg') || fn:endsWith(fileNameLower, '.png') || fn:endsWith(fileNameLower, '.gif')}">
                                                                                    <div
                                                    ...
                                                    <div class="card-footer">
                                                        ...
                                                        <c:if
                                                            test="${(not empty sessionScope.userId and post.userId eq sessionScope.userId) or (not empty sessionScope.role and sessionScope.role eq 'ADMIN')}">
                                                            <a href="${pageContext.request.contextPath}/funeral-reviews/${post.id}/update"
                        ...
                        <%-- 삭제 모달 (현재 로그인한 사용자가 글 작성자이거나 관리자일 경우에만 렌더링) --%>
                            <c:if
                                test="${not empty post and ((not empty sessionScope.userId and post.userId eq sessionScope.userId) or (not empty sessionScope.role and sessionScope.role eq 'ADMIN'))}">
                                <div class="modal fade" id="deleteModal" tabindex="-1"
                                    aria-labelledby="deleteModalLabel" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <form id="deleteForm"
                                                action="${pageContext.request.contextPath}/funeral-reviews/${post.id}/delete"
                                                method="POST">
